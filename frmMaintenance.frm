...
Begin VB.Form frmMaintenance 
   ...
   Begin VB.CheckBox chkSelectAll 
      ...
      Width           =   1065
   End
   Begin BiSAButtonProject.BiSAButton cmdCleanUpData 
      Height          =   405
      Left            =   2265
      TabIndex        =   0
      Top             =   5430
   ...
   Begin TrueOleDBGrid70.TDBGrid TDBGrid1 
      ...
      _StyleDefs(56)  =   ":id=39,.parent=33,.bgcolor=&HE6E6E6&"
   ...
   Begin VB.Label Label1 
      Caption         =   $"frmMaintenance.frx":0000
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
...
Private Sub GetLoadTable()
Dim db As New ADODB.Recordset
Dim n As Integer

  vaArray.ReDim 0, -1, 0, 1
  Set db = objData.Sql(GetDSN, "SELECT table_name FROM INFORMATION_SCHEMA.TABLES WHERE table_schema = '" & GetRegistry(reg_Database) & "'")
  If Not db.EOF Then
    Do While Not db.EOF
      vaArray.InsertRows vaArray.UpperBound(1) + 1
      n = vaArray.UpperBound(1)
      vaArray(n, 0) = 1
      vaArray(n, 1) = GetNull(db!table_name)
      If opt(1).Value = True Then
        Select Case vaArray(n, 1)
          Case "akun"
...
Private Sub cmdCleanUpData_Click()
Dim lSave As Boolean
Dim n As Integer
  
  If MsgBox("Apakah anda yakin akan melakukan hal ini?", vbYesNo + vbCritical) = vbYes Then
    ...
    For n = vaArray.LowerBound(1) To vaArray.UpperBound(1)
      If vaArray(n, 0) <> 0 Then
        lSave = IIf(lSave, objData.Delete(GetDSN, vaArray(n, 1), "", sisAssign, "", "1 = 1"), False)
      End If
    Next n
    ...
    If lSave Then
      objData.Save GetDSN
      GetNotifikasiRemove
      MsgBox "OK database anda telah berhasil dihapus"
    Else
      objData.Cancel GetDSN
      MsgBox "Maaf, database anda tidak berhasil dihapus"
    End If
  End If 'if msgbox
End Sub
