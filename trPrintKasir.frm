...
Begin VB.Form trPrintKasir 
   ...
   End
End
Attribute VB_Name = "trPrintKasir"
...
Dim n As Integer
    ...
    vp.StartDoc
      With vp
        ...
        .FontSize = 9
        .FontName = "Tahoma"
        .Text = "STRUK KASIR NO. " & noOrder & vbCrLf
'        .Text = "Tgl." & vbCrLf
        ...
        Set dbData = objData.Browse(GetDSN, "kasir k", "k.kodestock,s.nama,k.qty,k.harga,k.jumlah", "k.nomorkasir", sisAssign, noOrder, , , Array("left join stock s on s.kodestock = k.kodestock"))
        If Not dbData.EOF Then
          Do While Not dbData.EOF
            .TextAlign = taJustTop
            .Text = n + 1 & " - [" & GetNull(dbData!KodeStock) & "] " & GetNull(dbData!nama) & vbCrLf
            .Text = Padl(GetNull(dbData!qty), 5) & vbTab & " x " & Padl(Format(GetNull(dbData!Harga), "###,###,##0"), 14) & " = " & Format(GetNull(dbData!jumlah), "###,###,##0") & vbCrLf
            n = n + 1
            dbData.MoveNext
          Loop
        End If
        Garis
        .Text = "Subtotal" & vbTab & vbTab & Format(nSubTotal, "###,###,###,##0") & vbCrLf
        .Text = "Discount" & vbTab & vbTab & Format(nDiscount, "###,###,###,##0") & vbCrLf
        .Text = "Total" & vbTab & Format(nTotal, "###,###,##0") & vbCrLf
        .Text = "Tunai" & vbTab & Format(nCash, "###,###,##0") & vbCrLf
        .Text = "Kembali" & vbTab & vbTab & Format(nDiscount, "###,###,###,##0") & vbCrLf
'        .Text = "Kembali" & vbTab & Format(nChange, "###,###,##0") & vbCrLf
