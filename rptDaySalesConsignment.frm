...
Begin VB.Form rptDaySalesConsignment 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Days Sales Consignment"
   ClientHeight    =   2445
   ClientLeft      =   45
   ClientTop       =   345
   ClientWidth     =   7560
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame3 
      ...
      Begin BiSAButtonProject.BiSAButton cmdKeluar 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptDaySalesConsignment.frx":0000
      End
      Begin BiSAButtonProject.BiSAButton cmdPreview 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptDaySalesConsignment.frx":00A6
      End
      Begin BiSAButtonProject.BiSAButton BiSAButton1 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptDaySalesConsignment.frx":032C
      End
   End
End
Attribute VB_Name = "rptDaySalesConsignment"
...
Dim n As Integer
  ...
  cSQL = cSQL & " select s.kodesupplier,p.tgl,p.kodestock,s.nama,sum(p.qty) as total,sum(p.qty * p.hb) as saldo from penjualan p"
  ...
  cSQL = cSQL & " group by p.kodestock,p.tgl"
  ...
  If Not dbData.EOF Then
    Do While Not dbData.EOF
      ...
      vaArray(n, 4) = GetNull(dbData!Total)
      vaArray(n, 5) = GetNull(dbData!Saldo)
      dbData.MoveNext
    Loop
...
Dim n As Integer
  ...
  cSQL = ""
  cSQL = cSQL & " select s.kodesupplier,su.nama as namasupplier,p.tgl,p.kodestock,s.nama,sum(p.qty) as total,sum(p.qty * p.hb) as saldo from penjualan p"
  cSQL = cSQL & " left join stock s on s.kodestock = p.kodestock"
  cSQL = cSQL & " LEFT JOIN supplier su ON su.kodesupplier = s.kodesupplier"
  ...
  If Not dbData.EOF Then
    Do While Not dbData.EOF
      ...
      vaArray(n, 6) = GetNull(dbData!Saldo)
      dbData.MoveNext
    Loop
  
    With FrmRPT
      .AddPageHeader "Day Sales Consignment", tdbHalignCenter, , , , , 10, True
      .AddPageHeader "All Supplier", tdbHalignCenter, , , True, , 10, True
