...
Begin VB.Form mstHadiah 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Master Hadiah"
   ClientHeight    =   5850
   ClientLeft      =   45
   ClientTop       =   390
   ClientWidth     =   11400
   ...
   Begin TrueOleDBGrid70.TDBGrid TDBGrid1 
      ...
      Splits(0)._ColumnProps(13)=   "Column(2).Width=10134"
      Splits(0)._ColumnProps(14)=   "Column(2).DividerColor=0"
      Splits(0)._ColumnProps(15)=   "Column(2)._WidthInPix=10054"
...
Dim cInfo As String
  ...
  If MsgBox("Data Benar-benar dihapus ?" & vbCrLf & vbCrLf & cInfo, vbQuestion + vbYesNo) = vbYes Then
    If lExist(objData, "kartustockhadiah", "kodehadiah", GetKode) Then
      MsgBox "Maaf, data ini masih digunakan oleh sistem" & vbCrLf & "Tidak bisa dihapus"
      InitDel
      Exit Sub
    End If
    objData.Delete GetDSN, "masterhadiah", "kodehadiah", sisAssign, GetKode
  End If
  InitDel
End Sub
...
Private Sub cKode_Validate(Cancel As Boolean)
  Set dbData = objData.Browse(GetDSN, "masterhadiah", , "kodehadiah", sisAssign, GetKode)
...
Dim vaValue
  ...
  If ValidSaving() Then
    ...
    vaField = Array("kodehadiah", "nama", "poin")
    vaValue = Array(GetKode, cNama.Text, nPoin.Value)
    objData.Update GetDSN, "masterhadiah", "kodehadiah= '" & GetKode & "'", vaField, vaValue
    objData.Save GetDSN
    initvalue
    GetEdit False
  End If
...
Static Function ValidSaving() As Boolean
  ...
  If Not CheckData(nPoin.Value, "Poin Harus Diisi, Silahkan Mengulangi Pengisian") Then
    ValidSaving = False
    nPoin.SetFocus
    Exit Function
  End If
...
Private Sub GetMemory()
  Set dbData = objData.Browse(GetDSN, "masterhadiah s", "s.*", "s.kodehadiah", sisAssign, GetKode)
  If dbData.RecordCount > 0 Then
    cNama.Text = dbData!nama
    nPoin.Value = dbData!poin
  End If
End Sub
...
Dim n As Integer
  ...
  Set dbData = objData.Browse(GetDSN, "masterhadiah", "kodehadiah,nama,poin", "kodehadiah", sisContent, TDBGrid1.Columns(1).FilterText, " and nama LIKE '%" & TDBGrid1.Columns(2).FilterText & "%'")
  If Not dbData.EOF Then
    Do While Not dbData.EOF
      vaArray.InsertRows vaArray.UpperBound(1) + 1
      n = vaArray.UpperBound(1)
      vaArray(n, 0) = n + 1
      vaArray(n, 1) = GetNull(dbData!kodehadiah)
      vaArray(n, 2) = GetNull(dbData!nama)
      vaArray(n, 3) = GetNull(dbData!poin)
      dbData.MoveNext
    Loop
  End If
...
Dim db As New ADODB.Recordset
  ...
  Set db = objData.Browse(GetDSN, "masterhadiah s", "s.*", "s.kodehadiah", sisAssign, TDBGrid1.Columns(1).Text)
  If Not db.EOF Then
    cKode.Text = GetNull(db!kodehadiah)
    cNama.Text = GetNull(db!nama)
    nPoin.Value = GetNull(db!poin)
  End If
End Sub
