...
Begin VB.Form rptEstimasi 
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame1 
      ...
      Begin TrueDBReports60Ctl.TDBReports tdb 
         ...
         Fields(17).Name =   "cNamaSales"
         Fields(17).DisplayName=   "cNamaSales"
         Sections.Count  =   6
         ...
         Sections(0).Cells(2).Exp=   """LAPORAN DETAIL PENJUALAN"""
         ...
         Sections(1).Cells(6).Exp=   """Salesman "" & cNamaSales"
         ...
         Sections(4).Cells(0).Name=   "GrandTotal"
         Sections(4).Cells(0).Exp=   """Total"""
         ...
         Sections(4).Cells(1).Name=   "CELL_13"
         Sections(4).Cells(1).Exp=   """: Rp"""
         Sections(4).Cells(1).StyleExp=   "'Tdb_FooterGarisBawah'"
         Sections(4).Cells(1).Width=   5
         Sections(4).Cells(1).Height=   4
         Sections(4).Cells(1).AutoHeight=   0   'False
         Sections(4).Cells(2).Name=   "CELL_14"
         Sections(4).Cells(2).Exp=   "total"
         Sections(4).Cells(2).StyleExp=   "'Tdb_FooterGarisBawah'"
         Sections(4).Cells(2).Height=   4
         Sections(4).Cells(2).AutoHeight=   0   'False
         Sections(4).Cells(2).Format=   "###,###,##0.00"
         Sections(4).Cells(3).Name=   "CELL_12"
         Sections(4).Cells(3).Exp=   """Total Pembelian"""
         ...
         Sections(5).Cells(0).Name=   "CELL_23"
         Sections(5).Cells(0).Exp=   """TOTAL KEUNTUNGAN"""
         Sections(5).Cells(0).NewLine=   -1  'True
         Sections(5).Cells(0).Width=   35
         Sections(5).Cells(1).Name=   "CELL_24"
         Sections(5).Cells(1).Exp=   """ : """
         Sections(5).Cells(1).Width=   3
         Sections(5).Cells(2).Name=   "CELL_25"
         Sections(5).Cells(2).Exp=   "PIUTANGREF"
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame3 
      ...
      Begin BiSAButtonProject.BiSAButton cmdPreview 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptEstimasi.frx":00A6
      End
   End
End
Attribute VB_Name = "rptEstimasi"
...
Dim nTotalKeuntungan As Double
  ...
  End If
  cSQL = "SELECT p.nomorpenjualan,p.kodesatuan,s.barcode,p.kodestock,p.qty,p.harga,p.hb,p.discount as disc,p.jumlah,t.tunai,t.piutang,"
  cSQL = cSQL & " t.tgl,t.subtotal,t.discount,t.pajak,t.total,t.totalbiaya,"
  cSQL = cSQL & " t.kodeanggota,s.nama as namabarang,r.nama as namaanggota,sa.nama as namasalesman"
  cSQL = cSQL & " From estimasi p"
  cSQL = cSQL & " LEFT JOIN totestimasi t on t.nomorpenjualan = p.nomorpenjualan"
  cSQL = cSQL & " LEFT JOIN stock s on s.kodestock = p.kodestock"
  cSQL = cSQL & " LEFT JOIN anggota r on r.kodeanggota = t.kodeanggota"
  cSQL = cSQL & " LEFT JOIN salesman sa on sa.kodesalesman = t.kodesalesman"
  ...
  If Not dbData.EOF Then
    ...
    Do While Not dbData.EOF
      ...
      vaArray(n, 14) = (dbData!hb)
      'selisih margin
      vaArray(n, 8) = vaArray(n, 7) - vaArray(n, 14) '(dbData!jumlah)
      
      vaArray(n, 15) = (dbData!totalbiaya)
      vaArray(n, 16) = vaArray(n, 13) - vaArray(n, 15)
      vaArray(n, 17) = GetNull(dbData!namasalesman)
      
      nTotalKeuntungan = vaArray(n, 16)
    ...
    GetSUM nJumlah, nDiscount1, nDiscount2, nPajak, nTotal, Tunai, Piutang
    ...
    tdb.Parameters("PIUTANGREF").ValueExpression = nTotalKeuntungan
...
Private Sub GetSUM(ByRef nSubTotal As Double, ByRef nDisc As Double, _
                    ...
                    ByRef nTotal As Double, ByRef Tunai As Double, ByRef Piutang As Double)
...
Dim cWhere As String
  ...
  cSQL = "SELECT SUM(subtotal) as SubTotal,SUM(discount) as Disc, SUM(pajak) as Pajak,SUM(total) as Total,sum(tunai) as tunai,sum(piutang) as piutang "
  cSQL = cSQL & " FROM totestimasi"
  ...
  Set dbData = objData.Sql(GetDSN, cSQL)
  If Not dbData.EOF Then
    nSubTotal = dbData!Subtotal
    nDisc = dbData!Disc
    nDisc1 = 0
    nPajak = dbData!PAJAK
    nTotal = dbData!Total
    Tunai = dbData!Tunai
    Piutang = dbData!Piutang
  End If
End Sub
