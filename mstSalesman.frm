...
Begin VB.Form mstSalesman 
   ...
   Begin TrueOleDBGrid70.TDBGrid TDBGrid1 
      ...
      Splits(0)._ColumnProps(13)=   "Column(2).Width=6350"
      Splits(0)._ColumnProps(14)=   "Column(2).DividerColor=0"
      Splits(0)._ColumnProps(15)=   "Column(2)._WidthInPix=6271"
      Splits(0)._ColumnProps(16)=   "Column(2)._EditAlways=0"
      Splits(0)._ColumnProps(17)=   "Column(2)._ColStyle=512"
      Splits(0)._ColumnProps(18)=   "Column(2).Order=3"
      Splits(0)._ColumnProps(19)=   "Column(3).Width=7064"
      Splits(0)._ColumnProps(20)=   "Column(3).DividerColor=0"
      Splits(0)._ColumnProps(21)=   "Column(3)._WidthInPix=6985"
      Splits(0)._ColumnProps(22)=   "Column(3)._EditAlways=0"
      Splits(0)._ColumnProps(23)=   "Column(3)._ColStyle=516"
      Splits(0)._ColumnProps(24)=   "Column(3).Order=4"
      Splits(0)._ColumnProps(25)=   "Column(4).Width=2514"
      Splits(0)._ColumnProps(26)=   "Column(4).DividerColor=0"
      Splits(0)._ColumnProps(27)=   "Column(4)._WidthInPix=2434"
      ...
      _StyleDefs(66)  =   ":id=39,.parent=33,.bgcolor=&HE8E8E8&"
...
Dim lSave As Boolean
  ...
  If MsgBox("Data Benar-benar dihapus ?" & vbCrLf & vbCrLf & cInfo, vbQuestion + vbYesNo) = vbYes Then
    ...
    lSave = IIf(lSave, objData.Delete(GetDSN, "salesman", "kodesalesman", sisAssign, GetKode), False)
    If lSave Then
      objData.Save GetDSN
    Else
...
Private Sub cKode_Validate(Cancel As Boolean)
  Set dbData = objData.Browse(GetDSN, "salesman", , "kodesalesman", sisAssign, GetKode)
...
Private Sub cmdAdd_Click()
  ...
  cKode.SetFocus
  nPos = Add
  cKode.Text = GetNomorSalesman
End Sub

Private Function GetNomorSalesman() As String
GetNomorSalesman = ""

  Set dbData = objData.Browse(GetDSN, "salesman", "max(kodesalesman)+1 as nomorsalesman")
  If Not dbData.EOF Then
    If GetNull(dbData!nomorsalesman) = 0 Then
      GetNomorSalesman = Padl(1, 6, "0")
      Exit Function
    End If
    GetNomorSalesman = Padl(GetNull(dbData!nomorsalesman), 6, "0")
  End If
End Function
...
lSave = True
  ...
  If ValidSaving() Then
    ...
    vaField = Array("kodesalesman", "nama", "alamat", "telp")
    vaValue = Array(GetKode, cNama.Text, cAlamat.Text, cTelepon.Text)
    lSave = IIf(lSave, objData.Update(GetDSN, "salesman", "kodesalesman = '" & GetKode & "'", vaField, vaValue), False)
    If lSave Then
      objData.Save GetDSN
    Else
      objData.Cancel GetDSN
...
Private Sub GetMemory()
  Set dbData = objData.Browse(GetDSN, "salesman a", "a.*", "a.kodesalesman", sisAssign, GetKode)
  If dbData.RecordCount > 0 Then
    cNama.Text = GetNull(dbData!nama)
    cAlamat.Text = GetNull(dbData!alamat)
    cTelepon.Text = GetNull(dbData!telp)
  End If
End Sub
...
Dim n As Integer
  ...
  Set dbData = objData.Browse(GetDSN, "salesman", "kodesalesman,nama,alamat,telp", , , , , "kodesalesman")
  If Not dbData.EOF Then
    Do While Not dbData.EOF
      vaArray.InsertRows vaArray.UpperBound(1) + 1
      n = vaArray.UpperBound(1)
      vaArray(n, 0) = n
      vaArray(n, 1) = GetNull(dbData!kodesalesman)
...
Dim db As New ADODB.Recordset
  ...
  Set db = objData.Browse(GetDSN, "salesman a", "a.*", "a.kodesalesman", sisAssign, TDBGrid1.Columns(1).Text)
  If Not db.EOF Then
    cKode.Text = GetNull(db!kodesalesman)
