...
Begin VB.Form rptReturPenjualanGroupBySalesman 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Retur Penjualan Group By Salesman"
   ClientHeight    =   2235
   ClientLeft      =   45
   ClientTop       =   345
   ClientWidth     =   6990
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame1 
      ...
      Begin TrueDBReports60Ctl.TDBReports tdb 
         ...
         Parameters(1).Name=   "cSales"
         Parameters(2).Name=   "TGL2"
         Parameters(3).Name=   "TOTALJUMLAH"
         Parameters(3).Type=   5
         Parameters(4).Name=   "TOTALQTY"
         Parameters(4).Type=   5
         Parameters(5).Name=   "TOTALDISCOUNT1"
         Parameters(5).Type=   5
         Parameters(6).Name=   "TOTALDISCOUNT2"
         Parameters(6).Type=   5
         Parameters(7).Name=   "TOTALPAJAK"
         Parameters(7).Type=   5
         Parameters(8).Name=   "GRANDTOTAL"
         Parameters(8).Type=   5
         Fields.Count    =   15
         ...
         Sections(0).Cells(2).Exp=   """RETUR PENJUALAN"""
         ...
         Sections(0).Cells(5).Style.Font_Charset=   1
         Sections(0).Cells(5).Style.TextAlign=   3
         Sections(0).Cells(5).Style.TextVAlign=   0
         ...
         Sections(0).Cells(5).Style.fprops=   18874368
         ...
         Sections(5).Cells(5).Exp=   """SUB TOTAL PEMBELIAN (Rp)"""
         ...
         Sections(5).Cells(14).Exp=   """TOTAL RETUR"""
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame3 
      ...
      Begin BiSAButtonProject.BiSAButton cmdKeluar 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptReturPenjualanGroupBySalesman.frx":0000
      End
      Begin BiSAButtonProject.BiSAButton cmdPreview 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptReturPenjualanGroupBySalesman.frx":00A6
      End
   End
End
Attribute VB_Name = "rptReturPenjualanGroupBySalesman"
...
Dim db As New ADODB.Recordset
  ...
  Set db = objData.Browse(GetDSN, "salesman", "kodesalesman,nama", "nama", sisContent, cSalesman.Text)
  If Not db.EOF Then
...
Dim nTotal As Double
  ...
  End If
  cSQL = "SELECT p.nomorreturpenjualan,p.kodesatuan,p.kodestock,p.qty,p.harga,p.discount as disc,p.jumlah,"
  cSQL = cSQL & " t.tgl,t.subtotal,t.discount,t.pajak,t.total,tp.kodesalesman,"
  cSQL = cSQL & " t.kodeanggota,s.nama as namabarang,r.nama as namaanggota"
  cSQL = cSQL & " From returpenjualan p"
  cSQL = cSQL & " LEFT JOIN totrtnpenjualan t on t.nomorreturpenjualan = p.nomorreturpenjualan"
  cSQL = cSQL & " LEFT JOIN stock s on s.kodestock = p.kodestock"
  cSQL = cSQL & " LEFT JOIN anggota r on r.kodeanggota = t.kodeanggota"
  cSQL = cSQL & " LEFT JOIN totpenjualan tp on tp.nomorpenjualan = t.nomorpenjualan"
  cSQL = cSQL & " WHERE p.tgl >='" & Format(dDate(0).Value, "yyyy-MM-dd") & "'"
  cSQL = cSQL & " AND p.tgl <='" & Format(dDate(1).Value, "yyyy-MM-dd") & "'" & cFilter
  cSQL = cSQL & " AND tp.kodesalesman = '" & cSalesman.Text & "'"
  cSQL = cSQL & " ORDER BY p.nomorreturpenjualan"
  ...
  If Not dbData.EOF Then
    ...
    tdb.Parameters("cSales").ValueExpression = "'" & cNamaSalesman.Text & "'"
...
Dim cSQL As String
  ...
  cSQL = "SELECT SUM(subtotal) as SubTotal,SUM(discount) as Disc, SUM(pajak) as Pajak,SUM(total) as Total"
  cSQL = cSQL & " FROM totrtnpenjualan"
