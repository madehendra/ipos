...
Begin VB.Form mstKatalog 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Master Katalog"
   ClientHeight    =   5010
   ClientLeft      =   45
   ClientTop       =   375
   ClientWidth     =   6660
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame2 
      ...
      Begin TrueOleDBGrid70.TDBGrid DataGrid1 
         ...
         Splits(0)._ColumnProps(7)=   "Column(1).Width=6059"
         Splits(0)._ColumnProps(8)=   "Column(1).DividerColor=0"
         Splits(0)._ColumnProps(9)=   "Column(1)._WidthInPix=5980"
         ...
         _StyleDefs(56)  =   ":id=34,.wraptext=-1,.bold=-1,.fontsize=975,.italic=0,.underline=0"
         _StyleDefs(57)  =   ":id=34,.strikethrough=0,.charset=0"
         _StyleDefs(58)  =   ":id=34,.fontname=Times New Roman"
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame3 
      ...
      Begin BiSAButtonProject.BiSAButton cmdSimpan 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "mstKatalog.frx":07A6
      End
   End
End
Attribute VB_Name = "mstKatalog"
...
Dim cInfo As String
  ...
  If Trim(cKode.Text) <> "" Then
    If MsgBox("Data benar-benar dihapus...?" & vbCrLf & vbCrLf & cInfo, vbExclamation + vbYesNo) = vbYes Then
      If Not lExist(objData, "stock", "kodekatalog", cKode.Text) Then
        objData.Delete GetDSN, "katalog", "kodekatalog", sisAssign, cKode.Text
      Else
...
Dim vaField, vaValue
  ...
  If ValidSaving() Then
    vaField = Array("kodekatalog", "keterangan", "status")
    vaValue = Array(cKode.Text, cKeterangan.Text, GetOpt(optAktif))
    objData.Update GetDSN, "katalog", "kodekatalog = '" & cKode.Text & "'", vaField, vaValue
    GetSQL
    initvalue
    GetEdit False
   Else
...
Private Sub GetMemory()
  ...
  SetOpt optAktif, DataGrid1.Columns(2).Text
End Sub
...
Dim n As Single
  ...
  Set dbData = objData.Browse(GetDSN, "katalog", , , , , , "kodekatalog")
  If Not dbData.EOF Then
    Do While Not dbData.EOF
      vaArray.InsertRows vaArray.UpperBound(1) + 1
      n = vaArray.UpperBound(1)
      vaArray(n, 0) = GetNull(dbData!kodekatalog, "")
      vaArray(n, 1) = GetNull(dbData!keterangan, "")
      vaArray(n, 2) = GetNull(dbData!status)
      dbData.MoveNext
    Loop
  End If
