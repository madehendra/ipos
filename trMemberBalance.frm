...
Begin VB.Form trMemberBalance 
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame2 
      ...
      Begin BiSAButtonProject.BiSAButton cmdHapus 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "trMemberBalance.frx":0000
      End
      Begin BiSAButtonProject.BiSAButton cmdAdd 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "trMemberBalance.frx":028A
      End
      Begin BiSAButtonProject.BiSAButton cmdKeluar 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "trMemberBalance.frx":0435
      End
      Begin BiSAButtonProject.BiSAButton cmdSimpan 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "trMemberBalance.frx":04DB
      End
   End
   ...
   End
End
Attribute VB_Name = "trMemberBalance"
...
Dim n As Integer
  ...
  Set dbData = objData.Browse(GetDSN, "anggota a", "a.kodeanggota,a.nama,d.keterangan,m.balance", , , , , "d.keterangan,a.kodeanggota", Array("left join memberbalance m on m.kodeanggota = a.kodeanggota", "left join dep d on d.kodedep = a.kodedep"))
...
lSave = True
  ...
  For n = vaArray.LowerBound(1) To vaArray.UpperBound(1)
    FrmPB.RunPB
    lSave = IIf(lSave, UpdKodeTr(objData, msMemberBalance, vaArray(n, 4), Format(dTgl.Value, "yyyy-MM-dd"), aCfg(objData, msPiutangDagang), GetCostCenterUser(objData, GetRegistry(reg_username)), "Member Balance as " & vaArray(n, 1), vaArray(n, 3), 0, "", SNow), False)
    lSave = IIf(lSave, UpdKodeTr(objData, msMemberBalance, vaArray(n, 4), Format(dTgl.Value, "yyyy-MM-dd"), cRekeningLaba.Text, GetCostCenterUser(objData, GetRegistry(reg_username)), "Member Balance as " & vaArray(n, 1), 0, vaArray(n, 3), "", SNow), False)
  Next n
  FrmPB.EndPB
...
lSave = True
  ...
  objData.Start GetDSN
  For n = vaArray.LowerBound(1) To vaArray.UpperBound(1)
    lSave = IIf(lSave, objData.Delete(GetDSN, "bukubesar", "status", sisAssign, vbTrigger.msMemberBalance), False)
  Next n
  If lSave Then
    objData.Save GetDSN
  Else
...
lSave = True
  ...
  For n = vaArray.LowerBound(1) To vaArray.UpperBound(1)
    FrmPB.RunPB
    lSave = IIf(lSave, objData.Delete(GetDSN, "memberbalance", "memberbalanceid", sisAssign, vaArray(n, 4)), False)
    lSave = IIf(lSave, objData.Add(GetDSN, "memberbalance", Array("memberbalanceid", "kodeanggota", "balance"), Array(vaArray(n, 4), vaArray(n, 0), vaArray(n, 3))), False)
    lSave = IIf(lSave, UpdKartuHutang(objData, SisMemberBalance, vaArray(n, 4), Format(dTgl.Value, "yyyy-MM-dd"), vaArray(n, 0), "Member Balance as " & vaArray(n, 1), vaArray(n, 3), SNow, GetRegistry(reg_username)), False)
    lSave = IIf(lSave, objData.Delete(GetDSN, "totpenjualan", "nomorpenjualan", sisAssign, vaArray(n, 4)), False)
    
    lSave = IIf(lSave, objData.Add(GetDSN, "totpenjualan", _
    Array("nomorpenjualan", "kodeanggota", "username", "kodecostcenter", "tgl", "jthtmp", "subtotal", "total", "piutang", "flaglunas"), _
