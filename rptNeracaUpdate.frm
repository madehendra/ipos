...
Begin VB.Form rptNeracaUpdate 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Neraca Update"
   ClientHeight    =   2790
   ClientLeft      =   45
   ClientTop       =   345
   ClientWidth     =   6480
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame1 
      ...
      Begin TrueDBReports60Ctl.TDBReports RptNeraca 
         ...
         Fields(5).Name  =   "Persen"
         Fields(5).DisplayName=   "Persen"
         Sections.Count  =   6
         ...
         Sections(2).Cells(5).Name=   "Persen"
         Sections(2).Cells(5).Exp=   """%"""
         ...
         Sections(3).Cells(5).Name=   "Persen"
         Sections(3).Cells(5).Exp=   "Persen"
         Sections(3).Cells(5).StyleExp=   "'tdb_Number'"
         Sections(3).Cells(5).Width=   7
         Sections(3).Cells(5).CallExpression=   -1  'True
         Sections(4).Name=   "TotalAktiva"
         Sections(4).Type=   4
         Sections(4).Condition=   "Right(Kode, 1) = 'X'"
         Sections(4).StyleExp=   "'tdb_Total'"
         Sections(4).Tabulator=   "Detail"
         Sections(4).Cells.Count=   6
         ...
         Sections(4).Cells(5).Name=   "Persen"
         Sections(4).Cells(5).Exp=   "Persen"
         Sections(4).Cells(5).StyleExp=   "'tdb_NumberTotal'"
         Sections(4).Cells(5).CallExpression=   -1  'True
         ...
         Sections(5).Cells(2).Name=   "CELL_5"
         ...
         Sections(5).Cells(4).Exp=   """Pemeriksa"""
         ...
         Profiles(0).PrinterMarginTop=   700
         Profiles(0).PrinterMarginBottom=   700
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame2 
      ...
      Begin BiSAButtonProject.BiSAButton cmdKeluar 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptNeracaUpdate.frx":0000
      End
      Begin BiSAButtonProject.BiSAButton cmdPreview 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptNeracaUpdate.frx":00A6
      End
   End
End
Attribute VB_Name = "rptNeracaUpdate"
...
Dim cSQLCostCenter As String
  ...
  cSQL = cSQL & " Select '  ' as Kode,'Aktiva      ' as Keterangan,0,0,0,0 "
  cSQL = cSQL & " Union "
  cSQL = cSQL & " Select '1X' as Kode,'Total Aktiva' as Keterangan,0,0,0,0 "
  cSQL = cSQL & " Union "
  cSQL = cSQL & " Select '1Y' as Kode,'Pasiva      ' as Keterangan,0,0,0,0 "
  cSQL = cSQL & " Union "
  cSQL = cSQL & " Select 'XX' as Kode,'Total Pasiva' as Keterangan,0,0,0,0 "
  Set dbData = objData.SQL(GetDSN, cSQL)
  vaNeraca.LoadRows dbData.GetRows(dbData.RecordCount)
  
  ' Ambil Data Akun
  Set dbData = objData.Browse(GetDSN, "akun", "kodeakun,keterangan", "kodeakun", sisLT, "4", , "kodeakun")
  If Not dbData.EOF Then
    FrmPB.InitPB dbData.RecordCount + 1
    dbData.MoveFirst
    Do While Not dbData.EOF
      FrmPB.RunPB
      If Level(dbData!kodeakun) <= nLevel.value Then
        vaNeraca.InsertRows vaNeraca.UpperBound(1) + 1
        n = vaNeraca.UpperBound(1)
        vaNeraca(n, 0) = (dbData!kodeakun)
        vaNeraca(n, 1) = (RekSpace(dbData!kodeakun, dbData!keterangan))
        vaNeraca(n, 2) = 0
        vaNeraca(n, 3) = 0
        vaNeraca(n, 4) = 0
        vaNeraca(n, 5) = 0
...
'  Set dbData = objData.Browse(GetDSN, "SaldoAkun")
'  If Not dbData.EOF Then
'    dbData.MoveFirst
'    Do While Not dbData.EOF
'      If Left(dbData!Akun, 1) >= 4 And Left(dbData!Akun, 1) <= 5 Then
'        nAwalLaba = nAwalLaba + dbData!Awal
'      ElseIf Left(dbData!Akun, 1) <> 6 Then
'        cAkun = GetLevel(dbData!Akun, nLevel.Value)
'        n = vaNeraca.Find(0, 0, cAkun)
'        If n >= 0 Then
'          vaNeraca(n, 2) = vaNeraca(n, 2) + GS(dbData!Akun, dbData!Awal)
...
'  End If
  ...
  Set dbData = objData.Browse(GetDSN, "bukubesar", "kodeakun,Tgl,Sum(Debet) as Debet,Sum(Kredit) as Kredit", , , , " 1=1 AND tgl < '" & Format(dAwal.value, "yyyy-MM-dd") & "'" & cSQLCostCenter & " GROUP BY kodeakun")
  If Not dbData.EOF Then
    dbData.MoveFirst
    FrmPB.InitPB dbData.RecordCount
    Do While Not dbData.EOF
      FrmPB.RunPB
      cAkun = GetLevel(dbData!kodeakun, nLevel.value)
      If Left(dbData!kodeakun, 1) >= 4 And Left(dbData!kodeakun, 1) <= 5 Then
        nAwalLaba = nAwalLaba + dbData!debet - dbData!kredit
      ElseIf TypeRekening(dbData!kodeakun) <> SisAdministratif Then
        n = vaNeraca.Find(0, 0, cAkun)
        If n >= 0 Then
          vaNeraca(n, 2) = vaNeraca(n, 2) + GS(dbData!kodeakun, dbData!debet) - GS(dbData!kodeakun, dbData!kredit)
          vaNeraca(n, 4) = vaNeraca(n, 2) + vaNeraca(n, 3)
          vaNeraca(n, 5) = Format(Devide((vaNeraca(n, 4) - vaNeraca(n, 2)), vaNeraca(n, 2)) * 100, "###,###,##0.00")
        End If
      End If
      dbData.MoveNext
    Loop
  ...
  Set dbData = objData.Browse(GetDSN, "bukubesar", "kodeakun,Tgl,Sum(Debet) as Debet,Sum(Kredit) as Kredit", , , , " 1=1 AND tgl >= '" & Format(dAwal.value, "yyyy-MM-dd") & "' and tgl <= '" & SisFormat(dAkhir.value, Sis_yyyy_MM_dd) & "'" & cSQLCostCenter & " Group by kodeakun")
  If Not dbData.EOF Then
    FrmPB.InitPB dbData.RecordCount
    dbData.MoveFirst
    Do While Not dbData.EOF
      ...
      cAkun = GetLevel(dbData!kodeakun, nLevel.value)
      If Left(dbData!kodeakun, 1) >= 4 And Left(dbData!kodeakun, 1) <= 5 Then
        nLaba = nLaba + dbData!debet - dbData!kredit
      ElseIf TypeRekening(dbData!kodeakun) <> SisAdministratif Then
        n = vaNeraca.Find(0, 0, cAkun)
        If n >= 0 Then
          vaNeraca(n, 3) = vaNeraca(n, 3) + GS(dbData!kodeakun, dbData!debet) - GS(dbData!kodeakun, dbData!kredit)
          vaNeraca(n, 4) = vaNeraca(n, 2) + vaNeraca(n, 3)
          vaNeraca(n, 5) = Format(Devide((vaNeraca(n, 4) - vaNeraca(n, 2)), vaNeraca(n, 2)) * 100, "###,###,##0.00")
        End If
      End If
  ...
  ' Masukkan Akun Laba Tahun Berjalan Kedalam Neraca
  n = vaNeraca.Find(0, 0, GetLevel(aCfg(objData, msRekeningLaba), nLevel.value))
  If n >= 0 Then
    vaNeraca(n, 2) = vaNeraca(n, 2) + GS(aCfg(objData, msRekeningLaba), nAwalLaba)
    vaNeraca(n, 3) = vaNeraca(n, 3) + GS(aCfg(objData, msRekeningLaba), nLaba)
    vaNeraca(n, 4) = vaNeraca(n, 2) + vaNeraca(n, 3)
    vaNeraca(n, 5) = Format(Devide((vaNeraca(n, 4) - vaNeraca(n, 2)), vaNeraca(n, 2)) * 100, "###,###,##0.00")
  End If
  ...
  vaNeraca(1, 5) = Format(Devide((vaNeraca(1, 4) - vaNeraca(1, 2)), vaNeraca(1, 2)) * 100, "###,###,###,##0.00")

  
  vaNeraca(3, 2) = nAwalPasiva
  vaNeraca(3, 3) = nMutasiPasiva
  vaNeraca(3, 4) = nAkhirPasiva
  vaNeraca(3, 5) = Format(Devide((vaNeraca(3, 4) - vaNeraca(3, 2)), vaNeraca(3, 2)) * 100, "###,###,###,##0.00")
  ...
  Set dba = objData.Browse(GetDSN, "akun", , "jenis", sisAssign, "D")
  If Not dba.EOF Then
    Do While Not dba.EOF
      cAkun = GetLevel(dba!kodeakun, nLevel.value)
      n = vaNeraca.Find(0, 0, cAkun)
      If n > 0 Then
  ...
  With RptNeraca
    ...
    End If
    .Parameters(0).value = aCfg(objData, msNamaPerusahaan)
    .Parameters(1).value = aCfg(objData, msKota) & ", Tgl. " & Format(Date, "dd/MM/yy")
    .PrintPreview
  End With
  
  Dim cStatus As String
  GetNotifikasiAdd "Cek Balance Sheet"
  Set dbData = objData.Browse(GetDSN, "bukubesar", "sum(debet) as debet,sum(kredit) as kredit")
  If Not dbData.EOF Then
    If GetNull(dbData!debet) <> GetNull(dbData!kredit) Then
...
Private Function GS(ByVal cAkun As String, ByVal nValue As Double) As Double
  GS = IIf(Left(cAkun, 1) = "1", nValue, -nValue)
End Function

Private Sub dAkhir_Validate(Cancel As Boolean)
  If dAkhir.value < dAwal.value Then
    dAwal.value = dAkhir.value
  End If
End Sub
