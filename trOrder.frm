...
Begin VB.Form trOrder 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "FORM ORDERAN.."
   ClientHeight    =   8130
   ClientLeft      =   45
   ClientTop       =   375
   ClientWidth     =   12780
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame3 
      ...
      Begin BiSAButtonProject.BiSAButton cmdExportOrderan 
         Height          =   420
         Left            =   180
         TabIndex        =   0
         Top             =   120
...
lSave = True
  ...
  If ValidSaving Then
    ...
    vaField = Array("status", "tgl", "kodeanggota", "kodestock", "barcode", "debet", "username", "datetime")
    
    lSave = IIf(lSave, objData.Delete(GetDSN, "orderan", "tgl", sisAssign, Format(dTgl.Value, "yyyy-MM-dd"), " and kodeanggota = '" & cKodeMember.Text & "' AND status = 1"), False)
    For n = vaArray.LowerBound(1) To vaArray.UpperBound(1)
      vaValue = Array("1", Format(dTgl.Value, "yyyy-MM-dd"), cKodeMember.Text, vaArray(n, 3), vaArray(n, 1), vaArray(n, 4), GetRegistry(reg_UserName), SNow)
...
Dim nJumlah1, nQtyTmp As Double
    ...
    Set dbData = objData.Browse(GetDSN, "orderan o", "o.barcode,s.nama,o.kodestock,o.debet,s.hargajual", "o.kodeanggota", sisAssign, cMember, "and o.tgl = '" & Format(Tgl, "yyyy-MM-dd") & "' AND status = '" & Status & "'", , Array("LEFT JOIN stock s on s.kodestock = o.kodestock"))
    If Not dbData.EOF Then
    ...
    Else
      ...
      "tidak ada orderan di tgl " & Format(dTgl.Value, "dd-MM-yyyy"), vbExclamation
    End If
