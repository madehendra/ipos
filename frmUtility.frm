...
Begin VB.Form frmUtility 
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame1 
      ...
      Begin BiSAButtonProject.BiSAButton cmdPreview 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "frmUtility.frx":0000
      End
      Begin VB.Label Label1 
         Caption         =   "Please type your member name here..."
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
...
Dim vaAnggota As New XArrayDB
Dim vaBarang As New XArrayDB

Private Sub cmdPreview_Click()
Dim n As Integer

  vaAnggota.ReDim 0, -1, 0, 3
  Set dbData = objData.Browse(GetDSN, "anggota", "kodeanggota,nama,alamat", "nama", sisContent, cMember.Text, , "nama")
  If Not dbData.EOF Then
    Do While Not dbData.EOF
      vaAnggota.InsertRows vaAnggota.UpperBound(1) + 1
      n = vaAnggota.UpperBound(1)
      vaAnggota(n, 0) = GetNull(dbData!kodeanggota)
      vaAnggota(n, 1) = GetNull(dbData!nama)
      vaAnggota(n, 2) = GetNull(dbData!alamat)
      dbData.MoveNext
    Loop
  End If
  Set TDBGrid1.Array = vaAnggota
  TDBGrid1.ReBind
  TDBGrid1.Refresh
...
Dim n As Integer
  ...
  cSQL = ""
  cSQL = cSQL & " select distinct(p.kodestock),min(p.harga) as hargamin,max(p.harga) as hargamax,p.kodesatuan,s.nama,s.hargajual from totpenjualan t"
  cSQL = cSQL & " left join penjualan p on p.nomorpenjualan = t.nomorpenjualan"
  cSQL = cSQL & " left join stock s on s.kodestock = p.kodestock"
  cSQL = cSQL & " where t.kodeanggota = '" & TDBGrid1.Columns(0) & "'"
  ...
  If Not dbData.EOF Then
    Do While Not dbData.EOF
      ...
      vaBarang(n, 4) = GetNull(dbData!hargajual)
...
Private Function GetLastPrice(ByVal Kustomer As String, ByVal Stock As String, ByVal nIndex As Integer) As Double
Dim cSQL As String
Dim db As New ADODB.Recordset
  
  GetLastPrice = 0
  cSQL = ""
  cSQL = cSQL & " select p.nomorpenjualan,p.kodestock,s.nama,p.harga,p.tgl from totpenjualan t"
  cSQL = cSQL & " left join penjualan p on p.nomorpenjualan = t.nomorpenjualan"
  cSQL = cSQL & " left join stock s on s.kodestock = p.kodestock"
  cSQL = cSQL & " where p.kodestock = '" & Stock & "' and t.kodeanggota = '" & Kustomer & "'"
