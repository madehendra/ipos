...
Begin VB.Form trMemberTopUp 
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame3 
      ...
      Begin BiSAButtonProject.BiSAButton cmdSimpan 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "trTopUp.frx":0D40
      End
   End
End
Attribute VB_Name = "trMemberTopUp"
...
'          Exit Sub
  ...
  End If
  Set dbData = objData.Browse(GetDSN, "totmembertopup", "nomormembertopup,status", "tgl", sisAssign, Format(dTgl.value, "yyyy-MM-dd"), cFilterUsername)
...
Dim n As Single, cSQL As String
  ...
  cSQL = "Select m.kodeanggota,a.nama,m.keterangan,m.debet "
...
Private Sub cFaktur_Validate(Cancel As Boolean)
  Set dbData = objData.Browse(GetDSN, "membertopup", , "nomormembertopup", sisAssign, cFaktur.Text)
  If Not dbData.EOF Then
    Do While Not dbData.EOF
      If GetNull(dbData!lstatusid) <> "" Then
        MsgBox "Err. Data masih dipakai. Tidak bisa dihapus", vbCritical
        Cancel = True
        initvalue
        cFaktur.SetFocus
        Exit Sub
...
Dim nKe As Integer
  ...
  If Not GetValidDataBrowse(objData, "anggota", "kodeanggota", cKodeMember.Text) Then
    MsgBox "Member tersebut tidak ada dalam database "
    cKodeMember.SetFocus
    validOK = False
    Exit Function
  End If
  
  If nTopUp.value <= 0 Then
    MsgBox "Nilai Top Up yg masukkan tidak sah/tidak benar"
    nTopUp.SetFocus
    validOK = False
    Exit Function
  End If
...
Dim Faktur As String
  ...
  If ValidSaving() Then
    If MsgBox("Data benar-benar sudah VALID ?", vbYesNo) = vbYes Then
        ...
        lSave = IIf(lSave, objData.Update(GetDSN, "totmembertopup", "nomormembertopup = '" & Faktur & "'", Array("nomormembertopup", "tgl", "username", "status"), Array(Faktur, dTgl.value, GetRegistry(reg_Username), 0)), False)
        lSave = IIf(lSave, objData.Delete(GetDSN, "membertopup", "nomormembertopup ", sisAssign, Faktur), False)
        
        vaField = Array("nomormembertopup", "tgl", "kodeanggota", "keterangan", "debet", "lstatus")
        
        For n = 0 To vaArray.UpperBound(1)
          vaValue = Array(Faktur, dTgl.value, vaArray(n, 1), IIf(Trim(vaArray(n, 3)) = "", "Top Up " & vaArray(n, 2), vaArray(n, 3)), vaArray(n, 4), 0)
          lSave = IIf(lSave, objData.Add(GetDSN, "membertopup", vaField, vaValue), False)
          
          lSave = IIf(lSave, UpdKodeTr(objData, msMemberTopUp, Faktur, Format(dTgl.value, "yyyy-MM-dd"), Label1.Caption, "", "Top up " & vaArray(n, 3) & " an " & vaArray(n, 2), vaArray(n, 4), 0), False)
          lSave = IIf(lSave, UpdKodeTr(objData, msMemberTopUp, Faktur, Format(dTgl.value, "yyyy-MM-dd"), aCfg(objData, msRekeningTopUp), "", "Top Up " & vaArray(n, 3) & " an " & vaArray(n, 2), 0, vaArray(n, 4)), False)
        Next
...
'        If Not dbData.EOF Then
'          trPrint3.noOrder = Faktur
'          trPrint3.dTglTopUp = Format(GetNull(dbData!tgl), "dd.MM.yyyy")
'          trPrint3.cJenisTopUp = GetKeteranganTopUp
'          Load trPrint3
'          trPrint3.Show vbModal
'        End If
...
'      .WriteString Padc(Trim("SETORAN UANG"), 40) & vbCrLf
...
'      .WriteString "Print By: " & GetRegistry(reg_FullName) & vbCrLf
'      .WriteString "No. " & Faktur & Padl(Format(Now, "dd-MM-yyyy HH:MM:SS"), 22) & vbCrLf
'
'        Dim nTotalTopUp As Double
'
'        nTotalTopUp = 0
'        Set dbData = objData.Browse(GetDSN, "membertopup m", "m.kodeanggota,a.nama,m.keterangan,m.debet", "m.nomormembertopup", sisAssign, Faktur, , , Array("left join anggota a on a.kodeanggota = m.kodeanggota"))
'        If Not dbData.EOF Then
'          Do While Not dbData.EOF
'            .WriteString GetNull(dbData!kodeanggota) & " - " & GetNull(dbData!nama) & vbCrLf
'            .WriteString "Top Up " & GetNull(dbData!keterangan) & vbTab & vbTab & " = " & Padr(Format(GetNull(dbData!debet), "###,###,##0"), 15) & vbCrLf
'            n = n + 1
'            nTotalTopUp = nTotalTopUp + GetNull(dbData!debet)
'            dbData.MoveNext
'          Loop
'        End If
'
'      .WriteString Replicate("-", 40) & vbCrLf
'      .WriteString Padl("Total", 14) & vbTab & vbTab & " = " & Format(nTotalTopUp, "###,###,##0") & vbCrLf
'      .WriteString Replicate("-", 40) & vbCrLf
'      .WriteString "Terimakasih sudah melakukan Top Up" & vbCrLf
'      .WriteString "Simpanlah tanda bukti ini sebagai bukti setoran uang"
'
'      .Close
'
...
Private Function ValidSaving() As Boolean
  ...
  If vaArray.UpperBound(1) < 0 Then
    MsgBox "tidak ada data disimpan"
    ValidSaving = False
  End If
End Function
...
Private Sub Form_Activate()
  If Trim(aCfg(objData, msRekeningKasTopUp)) <> "" Then
    Label1.Caption = aCfg(objData, msRekeningKasTopUp)
  Else
    Label1.Caption = GetAkunKas(objData, GetRegistry(reg_Username))
  End If
End Sub
