...
Begin VB.Form mstCOD 
   ...
   Begin TrueOleDBGrid70.TDBGrid TDBGrid1 
      ...
      Splits(0)._ColumnProps(7)=   "Column(1).Width=7223"
      Splits(0)._ColumnProps(8)=   "Column(1).DividerColor=0"
      Splits(0)._ColumnProps(9)=   "Column(1)._WidthInPix=7144"
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame3 
      ...
      Begin BiSAButtonProject.BiSAButton cmdSimpan 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "mstCOD.frx":07A6
      End
   End
End
Attribute VB_Name = "mstCOD"
...
Dim cInfo As String
  ...
  If MsgBox("Data Benar-benar dihapus ?" & vbCrLf & vbCrLf & cInfo, vbQuestion + vbYesNo) = vbYes Then
    objData.Delete GetDSN, "tarifcod", "kode", sisAssign, GetKode
  End If
  initvalue
  GetLoadRows
  GetEdit False
...
Dim vaValue
  ...
  If ValidSaving() Then
    ...
    vaField = Array("kode", "nama", "harga")
    vaValue = Array(GetKode, cNama.Text, nHarga.Value)
    objData.Update GetDSN, "tarifcod", "kode = '" & GetKode & "'", vaField, vaValue
    objData.Save GetDSN
    initvalue
    GetEdit False
  End If
...
Private Sub GetMemory()
  Set dbData = objData.Browse(GetDSN, "tarifcod s", "s.*", "s.kode", sisAssign, GetKode)
  If dbData.RecordCount > 0 Then
    cKode.Text = GetNull(dbData!Kode)
    cNama.Text = GetNull(dbData!nama)
    nHarga.Value = GetNull(dbData!Harga)
  End If
End Sub
...
Dim n As Integer
  ...
  Set dbData = objData.Browse(GetDSN, "tarifcod s", "s.*", , , , , "s.kode asc")
  If Not dbData.EOF Then
    Do While Not dbData.EOF
      ...
      vaArray(n, 2) = GetNull(dbData!Harga)
      dbData.MoveNext
    Loop
  End If
...
Private Sub nMinQty_Validate(Cancel As Boolean)
  Set dbData = objData.Browse(GetDSN, "tarifcod", , "kode", sisAssign, GetKode)
  If dbData.RecordCount > 0 Then
    GetMemory
    If nPos = Delete Then
      HapusData
...
Dim db As New ADODB.Recordset
  ...
  Set db = objData.Browse(GetDSN, "tarifcod s", "s.*", "s.kode", sisAssign, TDBGrid1.Columns(0).Text)
  If Not db.EOF Then

    cKode.Text = GetNull(db!Kode)
