...
Begin VB.Form rptTagihan 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tagihan Bulanan..."
   ClientHeight    =   8220
   ClientLeft      =   45
   ClientTop       =   345
   ClientWidth     =   11820
   ...
   Begin SizerOneLibCtl.ElasticOne ElasticOne2 
      ...
      Begin TrueOleDBGrid70.TDBGrid tdbgrid1 
         ...
         Columns(1).Caption=   "ANGGOTA"
         Columns(1).DataField=   ""
         Columns(1)._PropDict=   "_MaxComboItems,516,2;_VlistStyle,514,3"
         Columns(2)._VlistStyle=   0
         Columns(2)._MaxComboItems=   5
         Columns(2).Caption=   "CICILAN"
         Columns(2).DataField=   ""
         Columns(2).NumberFormat=   "###,###,###,##0.00"
         Columns(2)._PropDict=   "_MaxComboItems,516,2;_VlistStyle,514,3"
         Columns(3)._VlistStyle=   0
         Columns(3)._MaxComboItems=   5
         Columns(3).Caption=   "BON"
         Columns(3).DataField=   ""
         Columns(3).NumberFormat=   "###,###,###,##0.00"
         Columns(3)._PropDict=   "_MaxComboItems,516,2;_VlistStyle,514,3"
         Columns(4)._VlistStyle=   0
         Columns(4)._MaxComboItems=   5
         Columns(4).Caption=   "ANG. PINJ"
         Columns(4).DataField=   ""
         Columns(4).NumberFormat=   "###,###,###,##0.00"
         Columns(4)._PropDict=   "_MaxComboItems,516,2;_VlistStyle,514,3"
         Columns(5)._VlistStyle=   0
         Columns(5)._MaxComboItems=   5
         Columns(5).Caption=   "SIM WAJIB"
         Columns(5).DataField=   ""
         ...
         Splits(0)._ColumnProps(9)=   "Column(1)._WidthInPix=4128"
         Splits(0)._ColumnProps(10)=   "Column(1)._EditAlways=0"
         Splits(0)._ColumnProps(11)=   "Column(1)._ColStyle=516"
         Splits(0)._ColumnProps(12)=   "Column(1).Order=2"
         Splits(0)._ColumnProps(13)=   "Column(2).Width=2910"
         ...
         Splits(0)._ColumnProps(19)=   "Column(3).Width=2858"
         ...
         Splits(0)._ColumnProps(25)=   "Column(4).Width=3069"
         ...
         Splits(0)._ColumnProps(31)=   "Column(5).Width=3466"
         Splits(0)._ColumnProps(32)=   "Column(5).DividerColor=0"
         Splits(0)._ColumnProps(33)=   "Column(5)._WidthInPix=3360"
         ...
         _StyleDefs(42)  =   "Splits(0).Columns(2).Style:id=32,.parent=13,.alignment=1"
         ...
         _StyleDefs(58)  =   "Splits(0).Columns(6).Style:id=58,.parent=13,.alignment=1"
   ...
   Begin SizerOneLibCtl.ElasticOne ElasticOne1 
      ...
      Begin BiSAButtonProject.BiSAButton cmdPreview 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptTagihan.frx":0000
      End
      Begin VB.Label Label1 
         Caption         =   "TAGIHAN BULAN"
         BeginProperty Font 
            Name            =   "Verdana"
            Size            =   8.25
            Charset         =   0
...
Dim nLebarAwalKolomAnggota  As Double
...
Dim n As Single
  ...
  nLebarAwalKolomAnggota = tdbgrid1.Columns(1).Width
End Sub

Private Sub GetLoadRows()
Dim n As Integer
    
  vaArray.ReDim 0, 0, 0, 7
  
  Set dbData = objData.Browse(GetDSN, "anggota", , , , , , "nama")
  If Not dbData.EOF Then
    Do While Not dbData.EOF
      ...
      vaArray(n, 2) = GetTagihanCicilan(objData, vaArray(n, 0), cListBulan.Text, cListTahun.Text)
      vaArray(n, 3) = GetSaldoPiutangByCustomer(vaArray(n, 0))
      vaArray(n, 4) = GetTagihanAngsuran(objData, vaArray(n, 0))
      vaArray(n, 5) = 0
      vaArray(n, 6) = vaArray(n, 2) + vaArray(n, 3) + vaArray(n, 4) + vaArray(n, 5)
      dbData.MoveNext
    Loop
  End If
...
Private Function GetTagihanCicilan(ByVal obj As CodeSuiteLibrary.Data, ByVal KodeAggota As String, ByVal Bulan, ByVal Tahun) As Double
Dim db As New ADODB.Recordset
Dim cSQL As String

  GetTagihanCicilan = 0
  
  cSQL = "select sum(c.jumlah) as jumlahcicilan from cicilan c"
  cSQL = cSQL & " left join totcicilan t on t.nomorcicilan = c.nomorcicilan"
  cSQL = cSQL & " Where c.Lunas = 0 And t.KodeAnggota = '" & KodeAggota & "' And c.bulan <= " & Bulan & " And c.tahun <= " & Tahun

  Set db = obj.Sql(GetDSN, cSQL)
  If Not db.EOF Then
    GetTagihanCicilan = GetNull(db!jumlahcicilan)
  End If
  
End Function

Private Function GetTagihanAngsuran(ByVal obj As CodeSuiteLibrary.Data, ByVal KodeAggota As String) As Double
Dim db As New ADODB.Recordset
Dim cSQL As String

  GetTagihanAngsuran = 0
  
  cSQL = "select sum(c.jumlah) as jumlahangsuran from angsuran c"
  cSQL = cSQL & " left join totpinjaman t on t.nomorpinjaman = c.nomorpinjaman"
  cSQL = cSQL & " Where c.Lunas = 0 And t.KodeAnggota = '" & KodeAggota & "' and c.bulan <=" & cListBulan.Text & " and c.tahun <= " & cListTahun.Text

  Set db = obj.Sql(GetDSN, cSQL)
  If Not db.EOF Then
    GetTagihanAngsuran = GetNull(db!jumlahangsuran)
  End If
End Function
...
Dim nSisaLebar As Double
    ...
    Me.Refresh
    nSisaLebar = tdbgrid1.Width - tdbgrid1.Columns(0).Width - tdbgrid1.Columns(2).Width - tdbgrid1.Columns(3).Width - tdbgrid1.Columns(4).Width - tdbgrid1.Columns(5).Width - tdbgrid1.Columns(6).Width
    If Me.WindowState = 2 Then
      tdbgrid1.Columns(1).Width = nSisaLebar - 1000
    Else
      tdbgrid1.Columns(1).Width = nLebarAwalKolomAnggota
    End If
End Sub
