...
Begin VB.Form trHapusMutasiSimpanan 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Hapus Mutasi Simpanan Harian..."
   ClientHeight    =   5055
   ClientLeft      =   45
   ClientTop       =   345
   ClientWidth     =   10695
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame3 
      ...
      Begin BiSAButtonProject.BiSAButton cmdKeluar 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "trHapusMutasiSimpanan.frx":0000
      End
      Begin BiSAButtonProject.BiSAButton cmdRefresh 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "trHapusMutasiSimpanan.frx":00A6
      End
      Begin BiSAButtonProject.BiSAButton cmdSimpan 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "trHapusMutasiSimpanan.frx":0250
      End
   End
End
Attribute VB_Name = "trHapusMutasiSimpanan"
    ...
    If MsgBox("Data Benar-benar Dihapus ?", vbQuestion + vbYesNo) = vbYes Then
      ...
      For n = 0 To xarray.UpperBound(1)
        If xarray(n, 0) = -1 Then
          'Hapus di table mutasi tabungan
          'Hapus di tabel bukubesar
          objData.Start GetDSN
          lSave = IIf(lSave, objData.Delete(GetDSN, "mutasitabungan", "nomormutasitabungan", sisAssign, xarray(n, 1)), False)
          lSave = IIf(lSave, DelKodeTr(objData, msSimpananHarian, xarray(n, 1)), False)
          If lSave Then
            objData.Save GetDSN
          Else
...
Private Function ValidSaving() As Boolean
  ...
  If Not CheckData(cKodeAnggota.Text, "Invalid kode rekening..!") Then
    ValidSaving = False
...
Dim n As Integer
  ...
  Set dbData = objData.Browse(GetDSN, "MutasiTabungan m", "0,m.nomormutasitabungan,m.Tgl,k.Keterangan,m.Jumlah,m.UserName", "m.Tgl", sisGTEqual, Format(dAwal.Value, "yyyy-MM-dd"), " and m.Tgl <='" & Format(dAkhir.Value, "yyyy-MM-dd") & "' and m.kodeanggota = '" & cKodeAnggota.Text & "'", "m.nomormutasitabungan", _
                              Array("Left join kodetransaksi k on k.Kodetransaksi = m.KodeTransaksi"))
  If Not dbData.EOF Then
    Do While Not dbData.EOF
      ...
      xarray(n, 2) = Format(GetNull(dbData!tgl), "dd/MM/yyyy")
      xarray(n, 3) = GetNull(dbData!Keterangan)
      xarray(n, 4) = GetNull(dbData!Jumlah)
      xarray(n, 5) = GetNull(dbData!UserName)
      dbData.MoveNext
    Loop
  End If
...
'  n = TDBGrid1.Bookmark
'  If n >= 0 Then
'    xarray(n, 0) = IIf(GetNull(xarray(n, 0)) <= 0, 1, 0) 'Not xarray(n, 0)
...
Private Sub InitValue()
  dAwal.Value = Date
  dAkhir.Value = Date
  Label1.Caption = "/tgl. " & Format(Date, "dd-MM-yyyy")
